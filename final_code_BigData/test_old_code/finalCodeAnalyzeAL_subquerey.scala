...
ORDER BY typeHighest DESC
    ...
    //which age group has the lowest deaths?
    val ageLowestDeathCount = spark.sql("""
        SELECT `Age Broad Group` as age , SUM(Total) as sum_total
        FROM cancer_data
        GROUP BY age
    ...
    //which age group out of this group had the highest death rates?
    val ageInEthnicHigestDeathCount = spark.sql("""
        SELECT ethnicity, age
        FROM (
            SELECT `Race and Hispanic Origin` as ethnicity, `Age Broad Group` as age, SUM(Total) as sum_total
            FROM cancer_data
            GROUP BY ethnicity, age
            ORDER BY ethnicity, sum_total DESC
        ) t
        GROUP BY ethnicity
        ORDER BY sum_total DESC
    """)
    ...
    println(s"Race/Ethnic group with overall higest death count: ")
    ...
    println(s"Age group with lowest: ")
    ageLowestDeathCount.show()
    println(s"Age group within the race/ethnic group with overall higest death count: ")
    ageInEthnicHigestDeathCount.show()
